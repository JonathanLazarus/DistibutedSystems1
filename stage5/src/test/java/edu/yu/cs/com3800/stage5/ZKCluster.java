...
public class ZKCluster {
    ...
    private void printLeaders(ArrayList<ZooKeeperPeerServerImpl> L) {
        for (ZooKeeperPeerServer server : L) {
            ...
            if (leader != null) {
                System.out.println(server.getClass().getSimpleName() + " on port " + server.getAddress().getPort() + " whose ID is " + server.getServerId() + " has the following ID as its leader: " + leader.getProposedLeaderID() + " and its state is " + server.getPeerState().name());
            }
        }
    }
    ...
    private void createServers(int N, int G) {
        ...
        for (int i = 1; i <= N; i++) {
            ...
            // we need to initialize servers with gatewayPeerServerID if we are making a gateway
            // the gateway is initialized with the first gatewayPeerServerImpl
            // whose ID will be N+1
            Long gwID = (this instanceof GatewayCluster) && G > 0 ? Long.valueOf(N+1) : null;
            ...
            ZooKeeperPeerServerImpl server =
                    new ZooKeeperPeerServerImpl(peerAddress.getPort(), 0, Integer.valueOf(i).longValue(), map, gwID, G);
        ...
        if (G > 0) {
            ...
            for (int i = N+1; i <= N+G; i++) {
                ...
                Long gwID = this instanceof GatewayCluster ? Long.valueOf(N+1) : null;
                GatewayPeerServerImpl server =
                        new GatewayPeerServerImpl(peerAddress.getPort(), 0, Integer.valueOf(i).longValue(), map, gwID, G);

...
public class Message {
    ...
    public Message(byte[] networkPayload) {
        if (networkPayload == null) throw new IllegalArgumentException("initiated Message with null networkPayLoad");
    ...
    public static Message getCollectCompletedWorkMsg(InetSocketAddress sender, InetSocketAddress target) {
        return new Message(MessageType.NEW_LEADER_GETTING_LAST_WORK, "".getBytes(),
                sender.getHostName(), sender.getPort(), target.getHostName(), target.getPort());
    }
    ...
    public boolean equals(Object obj) {
        ...
        try {
            return m.requestID == this.requestID && m.messageType == this.messageType && m.senderHost.equals(this.senderHost)
                    && m.senderPort == this.senderPort && m.receiverHost.equals(this.receiverHost) && m.receiverPort == this.receiverPort
                    && m.errorOccurred == this.errorOccurred && Arrays.equals(m.contents, this.contents);
        } catch (Exception e) {
            return false;
        }
    }

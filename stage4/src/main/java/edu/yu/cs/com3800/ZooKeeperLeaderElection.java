    ...
    public synchronized Vote lookForLeader() {
        ...
        while (myPeerServer.getPeerState() == LOOKING || myPeerServer.getPeerState() == OBSERVER) {
            try {
                ...
                if (msg == null) {
                ...
                } else if (!this.myPeerServer.isPeerDead(msg.getSenderAddress())) { // check if this server is a valid server
                    // todo stage5 if stmt ^
                    ...
                    switch (vote.getState()) {
                        ...
                        case LEADING:
                            ...
                            break;
                        case OBSERVER:
                            // messages from obsevers are ignored
    ...
    protected boolean haveEnoughVotes(Map<Long, ElectionNotification> votes, Vote proposal) {
        ...
        for (ElectionNotification vote : votes.values()) {
            if (vote.getProposedLeaderID() == proposal.getProposedLeaderID() && vote.getPeerEpoch() >= proposal.getPeerEpoch()) {
                logger.finer("vote " + vote.toString() + " from server #" + vote.getSenderID() + " is equal to proposal " + proposal.toString());
                counter++;
            }
